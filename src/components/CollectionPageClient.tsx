...
export default function CollectionPageClient({ initialCategory = "All", products, collectionMeta }: Props) {
  ...
  const handleQuickAdd = (product: Product) => {
    ...
    setTimeout(() => setQuickAddedId((cur) => (cur === product.id ? null : cur)), 1200);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-background px-4 py-16 sm:px-8">
      ...
      {/* Collection header */}
      <div className="mb-10 grid gap-6 rounded-4xl border border-[rgba(155,122,67,0.12)] bg-[rgba(255,250,241,0.88)] p-6 shadow-[0_24px_60px_rgba(61,47,28,0.08)] sm:p-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
        <div>
          <p className="text-[0.72rem] font-semibold uppercase tracking-[0.34em] text-primary">
            {collectionMeta?.badgeText || "Authentic edit"}
          </p>
          <h2 className="mt-3 font-serif text-3xl leading-tight text-foreground sm:text-4xl">
            {collectionMeta?.name || "Curated handbags with real-life wearability, clear structure, and premium finishes."}
          </h2>
          <p className="mt-4 max-w-2xl text-sm leading-7 text-(--color-muted) sm:text-base">
            {collectionMeta?.tagline || "Browse the collection with visible category counts, refined filtering, and product detail that feels closer to an editorial shopping room than a flat catalog."}
          </p>
          <div className="mt-5 flex flex-wrap gap-2 text-xs uppercase tracking-[0.22em] text-(--color-muted)">
        ...
        <div className="rounded-[1.6rem] border border-[rgba(155,122,67,0.12)] bg-white/70 p-4 shadow-[0_16px_36px_rgba(61,47,28,0.06)]">
          <div className="flex flex-wrap gap-4 text-sm">
            {[
              { label: "Selected", value: category },
              { label: "Shown", value: filtered.length },
              { label: "Sort", value: sort },
            ].map(({ label, value }) => (
              <div key={label}>
                <div className="text-[0.68rem] uppercase tracking-[0.24em] text-(--color-muted)">{label}</div>
      ...
      <div className="mb-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap justify-center gap-3 md:justify-start" role="group" aria-label="Filter by category">
          {categories.map((cat) => (
            <Button
              key={cat}
        ...
        <div className="flex items-center justify-center gap-3 md:justify-end">
          ...
          <Button
            ...
            className="min-w-40 cursor-pointer justify-between"
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-3">
        {[
          ...
          <div key={item.title} className="rounded-[1.4rem] border border-[rgba(155,122,67,0.12)] bg-[rgba(255,250,241,0.92)] p-5 shadow-[0_14px_32px_rgba(61,47,28,0.05)]">
            ...
            <p className="mt-2 text-sm leading-7 text-(--color-muted)">{item.description}</p>
          </div>
        ))}
      </div>
        ...
        {visibleProducts.map((product) => (
          ...
          >
            <div className="w-full max-w-xs">
              ...
              <p className="mt-2 text-center text-[0.72rem] uppercase tracking-[0.22em] text-(--color-muted)">
        ...
        {loading &&
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="flex flex-col items-center gap-2">
              <Skeleton variant="rectangular" width={180} height={180} sx={{ bgcolor: BRAND.border, borderRadius: 3 }} />
              <Skeleton width={120} height={24} sx={{ bgcolor: BRAND.border }} />
        ...
        <div className="mt-12 flex justify-center">
          <Button variant="secondary" onClick={handleLoadMore} className="px-8 text-base" disabled={loading}>
            {loading
              ...
              : `Load more${remainingCount > 0 ? ` (${remainingCount})` : ""}`}
          </Button>
        </div>
      )}
    </div>
